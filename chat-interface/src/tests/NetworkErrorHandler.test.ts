...
describe("NetworkErrorHandler", () => {
    ...
    describe("executeWithRetry", () => {
        ...
        it("does not retry on non-retryable errors", async () => {
            const authError = new NetworkError("Unauthorized", NetworkErrorCode.UNAUTHORIZED);
        ...
        it("respects timeout option", async () => {
            const mockRequest = vi.fn().mockImplementation(() => new Promise((resolve) => setTimeout(resolve, 200)));
    ...
    describe("createAbortController", () => {
        ...
        it("creates abort controller with timeout", () => {
    ...
    describe("Integration with real network scenarios", () => {
        it("handles fetch request with retry", async () => {
            ...
            const result = await NetworkErrorHandler.executeWithRetry(() => fetch("/api/test").then((r) => r.text()), {
