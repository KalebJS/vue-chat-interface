...
export class AudioController {
    ...
    async checkMicrophonePermission(): Promise<boolean> {
        try {
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                throw new AudioError("Media devices not supported", AudioErrorCode.NOT_SUPPORTED);
            }
    ...
    async startRecording(): Promise<void> {
        if (!this.audioState.isSupported) {
            throw new AudioError("Speech recognition not supported", AudioErrorCode.NOT_SUPPORTED);
        }

        if (!this.recognition) {
            throw new AudioError("Speech recognition not initialized", AudioErrorCode.RECORDING_FAILED);
        }
        ...
        try {
            ...
            const audioError =
                error instanceof AudioError
                    ? error
                    : new AudioError("Failed to start recording", AudioErrorCode.RECORDING_FAILED, error as Error);
    ...
    async speakText(text: string, voiceSettings?: VoiceSettings): Promise<void> {
        if (!this.synthesis) {
            throw new AudioError("Text-to-speech not supported", AudioErrorCode.NOT_SUPPORTED);
        }
        ...
        try {
            ...
            this.currentUtterance.onerror = (event) => {
                ...
                throw new AudioError(`Text-to-speech failed: ${event.error}`, AudioErrorCode.TEXT_TO_SPEECH_FAILED);
            };
            ...
            const audioError =
                error instanceof AudioError
                    ? error
                    : new AudioError("Failed to speak text", AudioErrorCode.TEXT_TO_SPEECH_FAILED, error as Error);
